...
type ProgramTvChannel struct {
	...
	MpsBackground        string `gorm:"column:mpsBackground;type:varchar(64);NOT NULL" json:"mpsBackground"`                       // ²¥·Å±³¾°Í¼
	...
	ChildId              int   `gorm:"column:childId;type:int(10) unsigned;default:0;NOT NULL" json:"childId"`                    // 子账号id-0主账号
	...
	PullStream           int   `gorm:"column:pullStream;type:int(1) unsigned;default:0;NOT NULL" json:"pullStream"`                   // ÊÇ·ñÀ­Á÷ 1ÊÇ 0·ñ
	...
	WatchNum             int   `gorm:"column:watchNum;type:int(10) unsigned;default:0;NOT NULL" json:"watchNum"`                      // 观看数量
	CustomWatchStatus    int   `gorm:"column:customWatchStatus;type:tinyint(1) unsigned;default:2;NOT NULL" json:"customWatchStatus"` // 是否开启自定义观看数据，2否1是
	IsDisplayWatchNum    int   `gorm:"column:isDisplayWatchNum;type:tinyint(1) unsigned;default:1;NOT NULL" json:"isDisplayWatchNum"` // 是否展示观看量1是2否
	StreamType           int   `gorm:"column:streamType;type:tinyint(1) unsigned;default:1;NOT NULL" json:"streamType"`
	ShareImg             string `gorm:"column:shareImg;type:varchar(64);NOT NULL" json:"shareImg"`
	...
	PlaybillType         int   `gorm:"column:playbillType;type:tinyint(1) unsigned;default:0;NOT NULL" json:"playbillType"` // 节目单状态1自动2自定义
	LssPublishUrl        string `gorm:"column:lssPublishUrl;type:varchar(255)" json:"lssPublishUrl"`
	IsVr                 int    `gorm:"column:isVr;type:int(1);default:0;NOT NULL" json:"isVr"`
}

func (m *ProgramTvChannel) TableName() string {
	return "program_tv_channel"
}

func (m *ProgramTvChannel) Create(Db *gorm.DB) error {
    err := Db.Model(&m).Create(&m).Error
    return err
}

func (m *ProgramTvChannel) Update(Db *gorm.DB, field ...string) error {
    sql := Db.Model(&m)
    if len(field) > 0 {
...
func (m *ProgramTvChannel) GetInfo(Db *gorm.DB) error {
    sql := Db.Model(m).Where("id = ? ", m.Id)
    err := sql.First(&m).Error
    return err
}

func GetTvInfo(Db *gorm.DB, id int) (ProgramTvChannel, error) {
	var info ProgramTvChannel
	err := Db.Model(&ProgramTvChannel{}).Where("id = ? AND status = 1", id).Find(&info).Error
	return info, err
}
