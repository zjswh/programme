...
func (m *ProgramTvPlaybill) TableName() string {
	return "program_tv_playbill"
}

func (m *ProgramTvPlaybill) Create(Db *gorm.DB) error {
	err := Db.Model(&m).Create(&m).Error
	return err
}

func (m *ProgramTvPlaybill) Update(Db *gorm.DB, field ...string) error {
	sql := Db.Model(&m)
	if len(field) > 0 {
...
func (m *ProgramTvPlaybill) GetInfo(Db *gorm.DB) error {
	sql := Db.Model(m).Where("id = ? ", m.Id)
	err := sql.First(&m).Error
	return err
}

func (m *ProgramTvPlaybill) GetList(Db *gorm.DB) ([]ProgramTvPlaybill, error) {
	var list []ProgramTvPlaybill
	err := Db.Model(m).Where("channelId = ? AND week = ? AND deleted = 0 AND type = ?", m.ChannelId, m.Week, m.Type).Find(&list).Error
	return list, err
}

func GetTvProgrammeInfo(Db *gorm.DB, channelId int, id int) (ProgramTvPlaybill, error) {
	var info ProgramTvPlaybill
	err := Db.Model(&ProgramTvPlaybill{}).Where("channelId = ? AND id = ? AND deleted = 0", channelId, id).Find(&info).Error
	return info, err
}

func GetTvProgrammeInfoById(Db *gorm.DB, id int) (ProgramTvPlaybill, error) {
	var info ProgramTvPlaybill
	err := Db.Model(&ProgramTvPlaybill{}).Where("id = ? AND deleted = 0", id).Find(&info).Error
	return info, err
}

func (m *ProgramTvPlaybill) Clear(Db *gorm.DB) error {
	err := Db.Model(m).
		Where("type = ? AND uin = ? AND channelId = ? AND week = ? ", m.Type, m.Uin, m.ChannelId, m.Week).
		Updates(ProgramTvPlaybill{Deleted: 1, DeleteTime: time.Now().Unix()}).Error
	return err
}

func DeleteTvPlayBill(Db *gorm.DB, idArr []int) error {
	err := Db.Model(ProgramTvPlaybill{}).Where("id IN (?)", idArr).
		Updates(ProgramTvPlaybill{Deleted: 1, DeleteTime: time.Now().Unix()}).Error
	return err
}
